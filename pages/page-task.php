...
if (isset($_POST['method'])) {
    if ($_POST['method'] == 'saveTask') {
        extract($_POST);
        if (!empty($name) && !empty($due_date) && !empty($project_id) && !empty($user_id)) {
            $task = new Task(null, $name, $description, 'unfinished', $project_id, $user_id, $due_date, null, null);

            $response = $taskService->createTask($task);
            if ($response) {
                $success = "Tâche créé avec succes";
                $tasks = $taskService->getTasks();
            } else {
    ...
    } elseif ($_POST['method'] == 'updateTask') {
        extract($_POST);
                            ...
                            <div class="d-flex flex-wrap align-items-center">
                                <div class="dropdown dropdown-project mr-3">
                                    <div class="dropdown-toggle" id="dropdownMenuButton03" data-toggle="dropdown">
                                        <div class="btn bg-body"><span class="h6">Statut :</span> <span id="selected-status">tâche</span><i class="ri-arrow-down-s-line ml-2 mr-0"></i></div>
                                    </div>
                                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton03">
                                        <a class="dropdown-item" href="#"><i class="ri-calendar-check-fill mr-2"></i>Terminé</a>
                                        <a class="dropdown-item" href="#"><i class="ri-chat-history-line mr-2"></i>Non terminé</a>
                                    </div>
                                </div>
                                <a href="#" class="btn btn-primary" data-target="#new-task-modal" data-toggle="modal">Nouvelle tâche</a>
                            </div>
                        </div>
                    </div>
                </div>
